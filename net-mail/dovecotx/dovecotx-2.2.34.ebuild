...
MY_P="dovecot-$(get_version_component_range 1-3)"
...
SRC_URI="https://dovecot.org/releases/${major_minor}/${rc_dir}${MY_P}.tar.gz
	sieve? (
	https://pigeonhole.dovecot.org/releases/${major_minor}/dovecot-${major_minor}-pigeonhole-${sieve_version}.tar.gz
	)
	managesieve? (
	https://pigeonhole.dovecot.org/releases/${major_minor}/dovecot-${major_minor}-pigeonhole-${sieve_version}.tar.gz
	) "
DESCRIPTION="An IMAP and POP3 server written with security primarily in mind - use libxcrypt"
...
IUSE_DOVECOT_SPECIAL="xcrypt"

IUSE="${IUSE_DOVECOT_AUTH} ${IUSE_DOVECOT_STORAGE} ${IUSE_DOVECOT_COMPRESS} ${IUSE_DOVECOT_OTHER} ${IUSE_DOVECOT_SPECIAL}"
...
src_prepare() {
	...
	if use xcrypt; then
		eapply -p0 "${FILESDIR}/dovecot-libxcrypt.patch"
	fi
